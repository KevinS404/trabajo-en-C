...
//Para modificar el archivo de este programa considere la primera linea
//como la tripleta 1 y la segunda como la tripleta 2

// El formato de solucion se representa mediante una matriz de 3x3,
// donde los 1 representan cuadrados negros o llenos y los 0 representan
// cuadrados blancos o vacioas


//La restriccion es que el numero de la columna sea el numero de la tripleta
//Es restriccion ya que asi podemos identificar las soluciones del problema
//Se valida porque de esta manera nos aseguramos de no pasarnos del valor buscado y entregar
//una solucion erronea
...
//necesitamos una variable global
...
//Funcion: agregar una matriz a la lista
...
int solucionRepetida(int ***soluciones, int elementos, int **matriz){
...
//Entrada: matriz bidimensional y arreglo tripleta con los numeros
//Salida: Retorna un entero
//funcion: Verifica si la suma de la columna es igual al numero en la tripleta, devolviendo
//un 1 en caso de coincidir o un 0 en caso de no coincidir
	...
	for(int i = 0;i < 3;i++){//primera fila
		...
		for (int j = 0; j < 3; ++j){
			...
			for (int k = 0; k < 3; ++k)
			{
				...
				if(verificador == 1){
					if(solucionRepetida(soluciones,elementos,identificador) == 0){//para verificar que no se repitan
						imprimir(identificador);
						representaciones++;//caso que no este en las soluciones
...
int main(int argc, char const *argv[])
{
	...
	printf("numero representaciones: %d \n", representaciones );
	if(representaciones == 1)
		printf("Hay solamente un caso.\n");
	else if(representaciones == 0){
		printf("No se encontro alguna solucion\n");
	}
	else{
		printf("Hay mas de una representacion posible \n");
	}
